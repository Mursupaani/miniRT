...
void	build_test_render(t_app *app)
{
	...
	w = world();
	w->light = point_light(point(-40, 50, 0), color(1, 1, 1));
	w->light->ambient_color = color (1, 1, 1);
	w->light->ambient_ratio = 0.2;
	c = camera(app->img->width, app->img->height, deg_to_rad(70));
	t_matrix4 c_trans = view_transform(point(0, 2, 20), point(0, 0, -1), vector(0, 1, 0), c);
	...
	add_transform(sphere, scaling_matrix4(12.6, 12.6, 12.6));
